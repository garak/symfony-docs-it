Come gestire il caricamento dei file con Doctrine
=================================================
...
il file nel controllore dopo aver gestito l'invio tramite una form. Per alcuni 
esempi in merito fare riferimento alla pagina 
:doc:`dedicata ai file type</reference/forms/types/file>`.
...
analizzati in questa sezione del cookbook.

Setup di Base
-------------

Innanzitutto creare una semplice classe Doctrine Entity su cui lavorare::
...
Per gestire il file attualmente caricato tramite la form utilizzare un campo
``file`` "virtuale". Per esempio, se si sta realizzando la form direttamente
...
.. note::
    ...
    automaticamente che il campo per la form sarà un file upload input. È per 
    questo motivo che non si rende necessario impostarlo esplicitamente al momento
    di creazione della form precedente (``->add('file')``).
...
Utilizzare ``id`` come nome del file
------------------------------------

Volendo utilizzare l'``id`` come nome del file, l'implementazione è leggermente
